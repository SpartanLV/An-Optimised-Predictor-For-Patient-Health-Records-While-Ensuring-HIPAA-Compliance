...
from .models import Patient, Event, Document, NEREntity, AuditLog
from .schemas import (
    PatientCreate, PatientOut,
    BulkEventsIn, EventOut, EventIn,
...
def create_patient(payload: PatientCreate, request: Request, db: Session = Depends(get_db), _: None = Depends(require_api_key), actor: str = Depends(actor_from_headers)):
    p = Patient(
        mrn=payload.mrn,
...
def list_patients(limit: int = 50, offset: int = 0, request: Request = None, db: Session = Depends(get_db), _: None = Depends(require_api_key), actor: str = Depends(actor_from_headers)):
...
def get_patient(patient_id: str, request: Request, db: Session = Depends(get_db), _: None = Depends(require_api_key), actor: str = Depends(actor_from_headers)):
...
def add_events_bulk(patient_id: str, payload: BulkEventsIn, request: Request, db: Session = Depends(get_db), _: None = Depends(require_api_key), actor: str = Depends(actor_from_headers)):
    p = db.get(Patient, patient_id)
    if not p:
        raise HTTPException(status_code=404, detail="Patient not found.")
...
def list_events(patient_id: str, request: Request, dt_from: Optional[str] = None, dt_to: Optional[str] = None, limit: int = 5000, db: Session = Depends(get_db), _: None = Depends(require_api_key), actor: str = Depends(actor_from_headers)):
    p = db.get(Patient, patient_id)
    if not p:
        raise HTTPException(status_code=404, detail="Patient not found.")
...
async def upload_document(
    ...
    # Store bytes to disk (optionally encrypted)
    ...
    # Parse extraction payload if present
    if extraction_json:
        try:
...
def list_audit_logs(
    ...
    # Avoid returning potentially sensitive meta blobs by default
    return [
        {
            "id": l.id,
